...
function copyYamlTemp(filesrc, nameFileDest) {
    ...
    const tmpdir = BaseGenerator.fs.mkdtempSync(path.join(os.tmpdir(), 'jhipster-'));
    ...
    BaseGenerator.fs.copySync(path.join(__dirname, filesrc), file);
    return file;
}

function deleteDirTemp(file) {
    BaseGenerator.fs.unlinkSync(file);
    BaseGenerator.fs.rmdirSync(path.dirname(file));
}
    ...
    describe('Test yaml property', () => {
        describe('Get YAML property', () => {
            ...
            it('get property on file that doesn\'t exist', () => {
                ...
                expect(() => utils.getYamlProperty(file, 'spring', BaseGenerator)).to.throw(/no such file or directory/);
            });
        ...
        describe('Add YAML properties', () => {
            it('add properties at beginin of file', () => {
                ...
                expect(result).eql('stream');
                const body = BaseGenerator.fs.readFileSync(`${file}`, 'utf8');
